...
\texttt{gpu/bin/mbs\_po\_gpu\_double\_fast} & FP64 & yes & Reference GPU
checks \\
\texttt{make\ -C\ gpu\ double\_debug} &
\texttt{gpu/bin/mbs\_po\_gpu\_double\_debug} & FP64 & yes & Debug help
and diagnostics \\
\end{longtable}
...
PO diffraction & \texttt{HandlerPO::ApplyDiffraction*},
\texttt{Handler::DiffractIncline*}, CUDA equivalents & Far-field Jones
...
\bottomrule\noalign{}
\endlastfoot
\texttt{Handler::DiffractIncline} & CPU scalar non-absorbing edge
integral \\
\texttt{Handler::DiffractInclineFast} & CPU optimized edge integral with
precomputed edge data \\
\texttt{Handler::DiffractInclineAbs} & Absorbing variant \\
...
4 & Droxtal & \texttt{L\ D\ extra}, uses the extra shape parameter \\
...
\texttt{-\/-adaptive} & \texttt{EPS} & Adaptive Sobol count & Doubles
orientations until relative convergence target. \\
...
\texttt{-\/-oldautofull} & \texttt{EPS} & Autofull search with oldauto
final grid & Useful when final regular grid is required. \\
\end{longtable}
...
bisection. \\
\texttt{-\/-auto\_phi} & none & Choose \texttt{Nphi} automatically from
size parameter. \\
...
\bottomrule\noalign{}
\endlastfoot
Theta grid & \texttt{-\/-tgrid} \textgreater{} \texttt{-\/-grid}
\textgreater{} \texttt{-\/-auto\_tgrid} \textgreater{} \texttt{-\/-auto}
\textgreater{} default \\
Phi grid & \texttt{-\/-nphi} \textgreater{} \texttt{-\/-grid}
\textgreater{} \texttt{-\/-auto\_phi} \textgreater{} \texttt{-\/-auto}
\textgreater{} default \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
Atomic Jones accumulation & \texttt{diffraction\_kernel} & Each thread
computes one beam/grid contribution and uses \texttt{atomicAdd} into the
complex Jones buffer. Afterwards \texttt{mueller\_batch\_kernel}
converts the summed Jones matrix to Mueller. & General fallback,
supports more combinations such as no-shadow output. \\
No-atomic grid kernels & \texttt{diffraction\_grid\_kernel} & Work is
...
Fused Mueller kernels & \texttt{diffraction\_grid\_mueller\_kernel},
\texttt{diffraction\_grid\_mueller\_full\_kernel},
\texttt{*\_full8\_kernel}, \texttt{*\_mixed8\_kernel} & The kernel
...
\texttt{reduce\_mueller\_orient\_kernel} & First writes per-orientation
...
\texttt{diffraction\_grid\_mueller\_multik\_kernel} & Computes several
...
\texttt{-\/-multikeq\_shared\_batches} & none & Batch nearby
...
\texttt{-\/-auto\_phi} & none & Automatic phi count. \\
...
and area cutoffs. \\
\texttt{-\/-beam\_cutoff\_j} & \texttt{EPS} & Skip beams by relative
` \\
\texttt{-\/-beam\_cutoff\_area} & \texttt{EPS} & Skip beams by relative
beam area. \\
\texttt{-\/-beam\_cutoff\_importance} & \texttt{EPS} & Skip beams by
relative ` \\
\texttt{-\/-trace\_cutoff} & \texttt{EPS} & Set trace pruning \texttt{J}
and area cutoffs. \\
\texttt{-\/-trace\_cutoff\_j} & \texttt{EPS} & Prune trace tree by
relative ` \\
\texttt{-\/-trace\_cutoff\_area} & \texttt{EPS} & Prune trace tree by
relative area. \\
\texttt{-\/-trace\_cutoff\_importance} & \texttt{EPS} & Prune trace tree
by ` \\
...
\texttt{MBS\_PARALLEL\_KEQ\_BATCH\_RATIO=R} & Environment default for
shared batch ratio. \\
\end{longtable}
