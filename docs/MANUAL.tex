...
binary forms their coordinate-dependent part in FP32 and uses
\texttt{sincosf}, while adding the FP64 centre phase only in the final
argument. \texttt{-{}-version} reports both phase precisions, the math profile,
...
The mixed phase-build optimization was checked separately on an RTX 3080 Ti
with hexagonal, concave, aggregate, and bullet-rosette particles up to
$L=1000\ \mu\mathrm{m}$. Relative to FP64 phase construction with the same
consumer \texttt{sincosf}, diffraction-kernel time decreased by 4.6--8.3\%
and complete GPU diffraction time by 4.4--7.9\%. The largest global Mueller
change was $4.3\times10^{-8}$; the largest relative change exactly at
$180^\circ$ was $3.7\times10^{-5}$. Complete FP32 phase construction was
faster but was rejected because its exact-backscatter error reached 0.68\%
for the bullet rosette.
...
thread per output (0) or one warp per output (1); incoherent automatic
mode also accounts for grid size and beams per orientation. \\
...
For coherent runs CUDA uses its reported FP32:FP64 throughput ratio. A ratio
of 16 or greater selects one thread per output; lower ratios retain warp
reduction. For incoherent runs one warp per output is selected when there are
at least 32 beams per orientation and no more than 200,000 orientation-grid
output cells. Other incoherent workloads use one thread per output. The common
scalar beam phase is omitted exactly using
$\operatorname{Mueller}(cJ)=|c|^2\operatorname{Mueller}(J)$; only the local
aperture phase is evaluated in the selected GPU storage precision. Coherent
Jones summation retains precise phase evaluation. With the default 64-thread
block the thread path also uses a $16\,\theta\times4\,\phi$ tiled grid when
incomplete edge tiles add no more than 15\% work. This removed two integer
divisions per output thread and changed wall time by 0--11\% in validation.
\texttt{MBS\_GPU\_TIMING=1} reports \texttt{reduction=thread|warp} and
\texttt{layout=3d|flat}.

For the compact thread path, automatic packing separates polygons with at most
four vertices from polygons with five to eight vertices. Larger polygons remain
in the general 32-vertex layout; one fused kernel consumes all three layouts.
Triangles and quadrilaterals use compile-time loop lengths when every compact
polygon has at least three vertices. Packing uses one stable bucket pass instead
of scanning the beam list once per vertex count. This is automatic in GPU builds.

On the RTX 3080 Ti concave absorbing probe this reduced summed FP32 CUDA-kernel
time by about 4\% and complete GPU diffraction time by about 3\%. For an
aggregate containing 3--10-vertex output polygons, FP32 kernel and complete GPU
diffraction time fell by 13\% and 14\%; the corresponding FP64 reductions were
4\% and 6\%. End-to-end gains are smaller when unchanged CPU tracing dominates.
The release gate compares all-compact and mixed paths with
\texttt{MBS\_GPU\_COMPACT\_BEAM4\_SPLIT=0} in both precision profiles.
...
\nolinkurl{MBS_GPU_TRACE_INFLIGHT_BEAMS} & 512 & Limit the combined number of trace beams submitted by all OpenMP workers; reduced after OOM. \\
...
\nolinkurl{MBS_GPU_TRACE_INFLIGHT_BEAMS} & Limit all OpenMP workers to a
combined in-flight trace budget; default \texttt{512}, minimum \texttt{32}. \\
