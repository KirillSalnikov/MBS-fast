...
\subsection{Speedup table}

\begin{tabular}{llrrrl}
\toprule
Test & Params & Old & New & Speedup & Accuracy \\
\midrule
Forward $0$--$25^\circ$ & $n\!=\!4$, $64\!\times\!43$ orient & 17\,min & 16\,sec & $\mathbf{63\times}$ & 0.007\% \\
Backward $160$--$180^\circ$ & $n\!=\!11$, $64\!\times\!43$ orient & 47\,min & 27\,sec & $\mathbf{105\times}$ & 0.8\% \\
\texttt{--coh\_orient} & $32\!\times\!22$ orient & 30\,sec & 0.5\,sec & $\mathbf{60\times}$ & $10^{-7}$ \\
Phase~1 only & $n\!=\!8$, 8192 orient & 5.2\,sec & 0.96\,sec & $\mathbf{5.4\times}$ & exact \\
\bottomrule
\end{tabular}

\subsection{Sources of speedup}

\begin{tabular}{lll}
\toprule
Optimization & Where & Effect \\
\midrule
Batched sincos (AVX-512/AVX2) & Phase~2 (diffraction) & $\sim$2$\times$ \\
Precomputed edge data & Phase~2 & $\sim$1.5$\times$ \\
Theta-coefficients (hoist A,B decomposition) & Phase~2 & $\sim$1.3$\times$ \\
OpenMP Phase~2 (thread-local Mueller) & Phase~2 & $N_\mathrm{cores}\times$ \\
OpenMP Phase~1 (CloneFor) & Phase~1 & $N_\mathrm{cores}\times$ \\
Adaptive theta grid (bisection) & Grid & 2--3$\times$ fewer points \\
Beam cutoff (two thresholds) & Phase~1+2 & skip 45--80\% beams \\
DiffractControlPoints (4~angles) & Adaptive & $\sim$100$\times$ cheaper convergence check \\
Incremental Sobol & Adaptive & $\sim$3.5$\times$ vs restart \\
Sub-chunked batches & Memory & prevents OOM for large $N$ \\
\bottomrule
\end{tabular}

\subsection{Accuracy notes}

\begin{itemize}
\item \texttt{--coh\_orient}: machine precision ($10^{-7}$) match with original code
\item \texttt{--incoh}: exact per-beam $|J|^2$ match
\item Default (incoherent averaging): $<0.01\%$ at convergence
\item Forward ($\theta\!=\!0$): 0.39\% from \texttt{d\_param} initialization fix
\item Backward ($n\!=\!11$): up to 0.8\% with 2860 orientations (converges with more)
\end{itemize}
