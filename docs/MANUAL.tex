...
\subsection{Multi-size (\texttt{--multigrid FILE})}

File with particle parameters (one per line, same \texttt{-p} format):
\begin{lstlisting}
1 50 6.25
1 100 12.5
1 200 25
...
Traces the largest particle once, recomputes diffraction for all sizes via \texttt{ComputeFromCache} (OpenMP parallel).
...
\item \texttt{ComputeFromCache} parallelized with OpenMP
