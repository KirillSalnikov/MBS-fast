...
| `make -C gpu fp32_consumer` | `gpu/bin/mbs_po_gpu_float_consumer` | FP32 | нет | Быстрый профиль потребительской GPU с FP32-тригонометрией фазы |
...
`GPU_PHASE_TRIG=precise|consumer`. Значение `consumer` допустимо только вместе
с FP32-хранилищем, использует отдельный каталог объектных файлов и само по себе
не включает `--use_fast_math`. Обычная команда сборки:
`make -C gpu fp32_consumer -j`. После сборки на целевой GPU выполните
...
FP32, добавляет общую FP64-фазу в итоговый аргумент и применяет `sincosf`.
Команда `--version` отдельно выводит точность построения
...
`L=20 мкм`, 192 ориентации в редуцированном SO(3), `N_phi=288`, `N_theta=144`.
...
|---|---:|---:|---:|---:|
| Точный FP64 | 8,56 с | 1,00 раза | эталон | эталон |
| Точный смешанный FP32 | 5,76 с | 1,49 раза | `5,68e-8` | `4,69e-8` |
| Потребительский FP32 | 3,25 с | 2,63 раза | `2,82e-7` | `2,73e-7` |

После калибровки для потребительских Ampere рекомендуется профиль
`fp32_consumer`: он в 1,77 раза быстрее точного смешанного FP32. В проверках
выпуклой и вогнутой геометрии, поглощения, `L=5...500 мкм` и 8--20 отражений
максимальная L2-ошибка M11 в боковой и задней областях составила `8,4e-5`;
для одного слабого внедиагонального элемента получено `5,3e-3`. Общий fast
math автоматически не выбирается: в этом тесте он был примерно на 9% медленнее
потребительского профиля и сильнее зависит от узкой интерференционной
структуры. FP64 остаётся режимом по умолчанию и обязательным эталоном при смене
частицы, показателя преломления, размера или сетки.

Смешанное построение фазы дополнительно проверено на RTX 3080 Ti для обычного
и полого столбиков, агрегата и пулевой розетки вплоть до `L=1000 мкм`.
Относительно прежнего построения всей фазы в FP64 при том же `sincosf` время
ядра дифракции уменьшилось на 4,6--8,3%, всего этапа GPU на 4,4--7,9%.
Максимальное глобальное изменение матрицы Мюллера составило `4,3e-8`, а
максимальное относительное изменение точно при 180 градусах `3,7e-5`.
Полностью FP32-построение было быстрее, но отклонено: для пулевой розетки его
ошибка при 180 градусах достигла 0,68%.
...
```bash
make -C gpu fp32_consumer -j GPU_ARCH=86 # Ampere: быстрый проверенный профиль
...
направления theta одной AVX2-операцией над интегралом по рёбрам. В каждой
...
соседним направлениям theta. Поэтому AVX2-цикл загружает четыре соседних
...
| `MBS_GPU_COMPACT_BEAM4_SPLIT=0` | Отключить отдельный формат на четыре вершины для треугольников и четырёхугольников и использовать прежний компактный формат на восемь вершин. |
| `MBS_GPU_WARP_BEAMS=0/1` | Переопределить автоматический выбор: один поток на выходную точку (`0`) или один warp (`1`). В когерентном режиме учитывается соотношение производительности FP32:FP64, а в некогерентном - число пучков и размер выходной сетки. |
| `MBS_GPU_WARP_GRID_3D=0` | Отключить специализацию трёхмерной сетки при активном warp-пути. |
| `MBS_GPU_THREAD_GRID_3D=0/1` | Переопределить плиточную трёхмерную сетку потокового пути. Автоматика включает её для блока из 64 потоков, если краевые плитки добавляют не более 15% работы. |
...
В когерентном режиме автоматический выбор использует сообщаемое CUDA отношение
производительности FP32 к FP64. При отношении 16 и выше выбирается компактное
ядро «один поток на выходную точку». В некогерентном режиме один warp на
выходную точку выбирается при наличии не менее 32 пучков на ориентацию и не
более 200 000 сочетаний «ориентация-точка сетки»; в остальных случаях
используется потоковое ядро. Общая скалярная фаза пучка исключается точно по
тождеству `Mueller(c J) = |c|^2 Mueller(J)`, а в выбранной точности GPU
вычисляется только локальная фаза апертуры. Когерентное суммирование матриц
Джонса сохраняет точное вычисление фазы.
При блоке из 64 потоков потоковый путь также использует плитку `16 theta
x 4 phi`, если неполные краевые плитки добавляют не более 15% работы. Это
убирает два целочисленных деления из каждого выходного потока; в проверенных
задачах изменение полного времени составило от 0 до 11% в сторону ускорения.
При `MBS_GPU_TIMING=1` выбор печатается как `reduction=thread|warp` и
`layout=3d|flat`.
...
Если в расчёте есть более крупные многоугольники, они остаются в общем формате
на 32 вершины; одно совмещённое ядро обрабатывает все три формата. Если у всех
компактных пучков не менее трёх вершин, циклы треугольников и четырёхугольников
имеют известную при компиляции длину. Один устойчивый проход по корзинам
заменяет повторное сканирование списка для каждого числа вершин. Дополнительный
флаг не нужен: это автоматический путь GPU по умолчанию.
...
уменьшилось на 13%, всей GPU-дифракции на 14%; для FP64 выигрыш составил 4% и
6% соответственно. Полный выигрыш меньше, когда преобладает неизменённая
CPU-трассировка. Выпускной тест сверяет оба пути, полностью компактный и
смешанный, с `MBS_GPU_COMPACT_BEAM4_SPLIT=0` для обеих точностей.
...
Если видимо несколько GPU, рабочие потоки OpenMP циклически назначаются тому же
набору устройств, который задан `MBS_GPU_MULTI` и `MBS_GPU_MULTI_MAX` для
дифракции. У каждой карты отдельны CUDA workspace, поток и лимит одновременно
обрабатываемых пучков. Поэтому CUDA-подготовка трассировки и дифракция используют
все выбранные карты без запуска нескольких полных копий расчёта. Точные
пересечения и разделение пучков по-прежнему выполняются на CPU.
...
адресу карты. Рабочие потоки OpenMP внутри одного процесса перекрываются в
отдельных потоках CUDA, но совместно используют общий предел в 512 пучков.
Поэтому пакеты отдельных потоков не могут бесконтрольно умножить объём рабочих
буферов и резерва стека CUDA. После CUDA OOM вдвое уменьшаются и пакет, и общий
предел. Эти ограничения не блокируют ядра дифракции.
...
| `MBS_GPU_TRACE_INFLIGHT_BEAMS=N` | `512` | Максимальное суммарное число пучков в CUDA-трассировке всех OpenMP-потоков; после OOM уменьшается автоматически. |
...
| `MBS_GPU_TRACE_MIN_CANDIDATES=N` | `1024` для одной GPU, `256` для нескольких | Минимальное общее число кандидатов для отправки пакета на GPU. Автоматический многокарточный порог проверен на пяти V100; ручное значение требует повторного измерения для другой геометрии. |
...
| `MBS_GPU_COMPACT_BEAM4_SPLIT=0` | Отключить отдельный четырёхвершинный формат треугольников и четырёхугольников. |
| `MBS_GPU_WARP_BEAMS=0/1` | Переопределить автоматическую редукцию: один CUDA-поток на выходную точку (`0`) или один warp (`1`); в некогерентном автоматическом режиме также учитываются размер сетки и число пучков на ориентацию. |
| `MBS_GPU_WARP_GRID_3D=0` | Отключить специализацию трёхмерной сетки при активной warp-редукции. |
| `MBS_GPU_THREAD_GRID_3D=0/1` | Переопределить автоматическую плиточную трёхмерную сетку потокового пути на потребительских GPU. |
...
| `MBS_GPU_TRACE_INFLIGHT_BEAMS=N` | Общий предел одновременно трассируемых пучков процесса; по умолчанию `512`, минимум `32`. |
...
| `MBS_GPU_TRACE_EXACT_HIT=1` | Экспериментально вычислять первое точное пересечение пучка с гранью на GPU; по умолчанию выключено. |
| `MBS_GPU_TRACE_PARTITION=1` | После точного попадания вычислять на GPU достигнутую апертуру и дополнение исходного многоугольника; требует `MBS_GPU_TRACE_EXACT_HIT=1`. Однозначный быстрый формат хранит до 20 вершин и одну остаточную часть; разбиение на несколько частей, переполнение и нарушение баланса площади дают локальный CPU-fallback. |
| `MBS_GPU_TRACE_PARTITION_MIN_ITEMS=N` | Минимальная ширина уровня для GPU-разбиения; более узкие пакеты сразу используют CPU. По умолчанию `64`; уменьшение порога увеличивает число запусков ядра и передач малых буферов. |
| `MBS_GPU_TRACE_VERIFY_EXACT=1` | Повторять на CPU каждое точное GPU-пересечение и разбиение; проверочный запуск использует независимо вычисленное CPU-разбиение как физический результат. |
| `MBS_GPU_TRACE_LEVEL_QUEUE=1` | Обрабатывать активные пучки пакетами с одинаковым числом отражений (`current` перед `next`); по умолчанию выключено. |
