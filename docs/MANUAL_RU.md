...
Управляющая C++-часть CUDA-сборки теперь требует ровно один профиль FP32/FP64.
Бинарный файл со строкой `unknown-precision` является устаревшим или собранным
в обход поддерживаемых профилей и не должен использоваться для расчётов.
...
Оптимизированные CPU- и CUDA-пути используют поперечность `vf` и
`vt = vf x direction`, поэтому продольно спроецированные временные векторы
строить не требуется. Обозначив `s = direction`, `X_T = n x D_T` и
`X_P = n x D_P`, матрицу поворота вычисляют непосредственно:
...
Здесь `*` означает скалярное произведение. Исключённый член параллелен `s`,
а оба вектора выходного базиса перпендикулярны `s`. Поэтому преобразование
является точным следствием тождества для смешанного произведения, а не
приближением поляризации. Оно убирает построение двух временных векторов и их
продольных проекций для каждого вклада пучка в направление наблюдения.
