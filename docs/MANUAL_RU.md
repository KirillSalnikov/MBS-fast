...
| `MBS_GPU_WARP_BEAMS=0/1` | Переопределить автоматический выбор: один поток на выходную точку (`0`) или один warp (`1`). По умолчанию потребительские GPU используют потоковый путь, а вычислительные GPU с сильным FP64 - warp-путь. |
...
Автоматический выбор использует сообщаемое CUDA отношение производительности
FP32 к FP64. При отношении 16 и выше выбирается компактное ядро «один поток на
выходную точку». На RTX 3080 Ti это сократило полное время на 2-11% в тестах
выпуклой и вогнутой частиц, FP32/FP64 и глубины 8-20 отражений. Для V100/A100
сохраняется warp-редукция.
При блоке из 64 потоков потребительский путь также использует плитку `16 theta
...
| `MBS_GPU_WARP_BEAMS=0/1` | Переопределить автоматическую редукцию: один CUDA-поток на выходную точку (`0`) или один warp (`1`). |
