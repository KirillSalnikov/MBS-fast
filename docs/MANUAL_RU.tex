...
списков. Размер пакета одного рабочего потока выбирается как
\[
  B=\max\!\left(64,
...
пакетов и ориентаций процесса. На сложной ориентации глубоко вогнутой
...
\nolinkurl{MBS_GPU_TRACE_NONBLOCKING_STREAM} & 0 & Неблокирующий поток CUDA для каждого worker; только для профилирования. \\
...
\code{MBS_GPU_WARP_BEAMS=0} & Вернуть последовательный обход пучков одним CUDA-потоком вместо стандартной warp-редукции. \\
\code{MBS_GPU_WARP_GRID_3D=0} & Отключить только специализацию warp-пути для трёхмерной сетки. \\
...
\nolinkurl{MBS_GPU_TRACE_NONBLOCKING_STREAM=0/1} & Неблокирующие потоки CUDA; по умолчанию 0, только для профилирования. \\
...
фазы остаются FP64. Команда \code{--version} выводит фактический профиль
хранения, фазы, математических функций и архитектуры GPU.

На RTX 3080 Ti для характерного поглощающего вогнутого теста
($k_{\mathrm{eq}}=20$, 64 ориентации SO(3), $N_\phi=720$,
$N_\theta=360$) получены следующие медианные времена:
...
\midrule
\endhead
Точный FP64 & 4,92 с & 1,00 & контрольный результат \\
Точный смешанный FP32 & 3,45 с & 1,43 & общая $L_2=1{,}00\cdot10^{-6}$ \\
FP32 с быстрыми функциями & 3,63 с & 1,36 & общая $L_2=1{,}02\cdot10^{-6}$ \\
\bottomrule
\end{longtable}

Поэтому для потребительской GPU рекомендуется точный смешанный FP32 после
проверки на целевом классе частиц и диапазоне размеров. Быстрые функции не
включаются автоматически: в этом тесте они оказались немного медленнее и не
дали выигрыша в точности. Контрольные точки, сильное поглощение и глубокие
вогнутые траектории следует выборочно пересчитывать в FP64.
