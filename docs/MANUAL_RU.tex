...
расчёт с полным диапазоном $\gamma$. $N$ остаётся числом реально трассируемых
ориентаций $\beta,\gamma$. Для сохранения плотности узлов можно примерно вдвое
уменьшить $N$: например, использовать
\code{--so3-quaternion 2048 --mirror-gamma} вместо
\code{--so3-quaternion 4096}.
