...
эквивалентного усреднения. Режим \code{--so3-quaternion} непосредственно
строит выборку в полной группе вращений $SO(3)$.
...
Файл \code{particle_for_check.dat} создаётся автоматически в каталоге
результата. Параметр \code{--save-geometry} удобнее, когда путь должен быть
известен заранее.
...
\midrule
\endhead
\code{--fixed-orientation B G} & Одна ориентация $\beta,\gamma$; удобна для отладки CPU/GPU. \\
\code{--orientation-file FILE} & Ориентации из файла. \\
...
\code{--so3-quaternion N} & Выборка Хаммерсли непосредственно на $SO(3)$ через кватернионы. \\
...
Автоматическая сетка рассеяния требует \code{--method po} и дифракционной
сетки ориентаций. Она конфликтует с явными \code{--scattering-grid},
...
\midrule
\endhead
\code{gpu_float} & \code{gpu/bin/mbs_po_gpu_float} & Одинарная точность без \code{--use_fast_math}. \\
\code{gpu_float_fast} & \code{gpu/bin/mbs_po_gpu_float_fast} & Наибольшая пропускная способность; требует калибровки ошибки. \\
\code{gpu_double_fast} & \code{gpu/bin/mbs_po_gpu_double_fast} & Двойная точность с CUDA \code{--use_fast_math}; основной рабочий вариант FP64. \\
...
Наиболее строгий контрольный GPU-бинарный файл двойной точности без быстрых
математических функций собирается отдельно:
...
\code{--orientation-file} & \code{FILE} & Прочитать ориентации из файла. \\
...
\code{--so3-quaternion} & \code{N} & Выборка полной группы $SO(3)$ через кватернионы. \\
...
\code{--trace-max-beams} & \code{N} & Прервать ориентацию после $N$ узлов дерева; 0 отключает предел. \\
...
\code{FILE} из \code{--save-geometry} & Рабочая геометрия по явно выбранному пути. \\
...
предварительные расчёты, не контрольные. \\
Старое значение & 0 & 0 & 100 & Совместимость, если профиль и отдельные
пороги не указаны. \\
\bottomrule
\end{longtable}

Профили используют совместную важность $|J|^2A$, а не независимые пороги по
амплитуде и площади. Поэтому большая слабая или малая интенсивная ветвь не
удаляется только по одному множителю. Без явных параметров сохраняется старое
упрощение с отношением площадей 100. Только \code{--cutoff-profile off}
...
\code{--trace-max-beams N} & $N\ge0$ & Жёсткий предел дерева одной ориентации; 0 отключает его. Достижение предела означает неполный результат. \\
\bottomrule
\end{longtable}
...
Платформа & \code{gpu/bin/mbs_po_gpu_double_fast}. \\
