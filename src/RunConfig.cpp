...
void ValidateScan(const ArgPP &args, const RunConfig &config)
{
    ...
    {
        if (args.IsCatched("multigrid")
            && !SupportsSerialDmaxScan(config.orientation) && !args.IsCatched("extinction_only"))
        ...
        if ((args.IsCatched("multikeq") || args.IsCatched("multikeq_list"))
            && !SupportsSerialKeqScan(config.orientation) && !args.IsCatched("extinction_only"))
                                     ...
                                     bool cudaCompiled)
{
    ...
    if (args.IsCatched("extinction_only"))
    {
        if (config.method != RunMethod::PhysicalOptics || config.useGpu
            ...
            Fail("--extinction-only requires PO, CPU, explicit convex geometry and Hammersley orientations.",
        ...
        for (const char *key : incompatible)
            if (args.IsCatched(key))
                Fail("--extinction-only does not support this modifier: " + std::string(key),
                     "remove it; extinction-only uses the full orientation domain and physical forward amplitude.");
    }
